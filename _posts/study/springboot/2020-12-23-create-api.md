...
지금까지 생성한 프로그램의 GET / POST API 들이 모두 Open API 에 해당한다고 볼 수 있습니다. 물론 서버에 직접 호출하기 위해서 서버의 포트를 열어주고, 방화벽을 넘어서도 호출할 수 있도록 인프라 요소를 손봐야 하는 것은 당연합니다. 또한 지나친 요청으로 인해 서버가 다운되는 것을 방지하기 위한 G/W 요소도 조금은 필요할 수도 있습니다. 

#### API 설계에 대한 몇 가지

경험적으로 보았을 때에 API를 설계한다고 하면 `/api/v1` 과 같은 <mark style='background-color: #ffdce0'>prefix</mark> 를 갖습니다. 다른 API 형태도 있을 것이기 때문에 `api` 라는 것을 맨 앞에 두고, Open API 의 특성 상 버전이 업그레이드 될 수 있으므로 `v1` 과 같은 버전명을 넣어줍니다. 정답은 없고 관리적인 측면을 고려해서 자유롭게 만들면 되지만, 업계의 보이지 않는 룰 같은 형태로 통용되는 부분은 따라주는 것이 좋아 보입니다. 
